...
export function useScheduling() {
  ...
  const fetchActivities = useCallback(async (date, plan = false) => {
    ...
    try {
      ...
      const result = await schedulingService.getByDate(dateStr, plan)
  ...
  const createActivity = useCallback(async (data) => {
    const event = await schedulingService.create(data)
    setActivities((prev) => {
  ...
  const updateActivity = useCallback(async (id, data) => {
    const event = await schedulingService.update(id, data)
    setActivities((prev) => {
