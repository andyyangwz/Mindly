...
export default function JournalForm({ form, setForm, editId, onSave, onBack, folders, selectedFolderIds, onFolderIdsChange }) {
  ...
  const callTransform = async (endpoint, label) => {
    ...
    const input = preserveStructure
      ? { html: selectedHTML }
      : { text: selectedText }
    const raw = preserveStructure ? selectedHTML : selectedText
    if (!raw.trim()) {
      setVoiceError("Selected text is empty. Nothing to transform.")
      return
    }
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${buttonsDisabled ? theme.border : theme.border}`,
                  background: buttonsDisabled ? "var(--color-card, white)" : "var(--color-card, white)",
                  color: buttonsDisabled ? theme.muted : theme.dark,
              ...
              <button
                ...
                style={{
                  display: "flex", alignItems: "center", gap: 5,
                  padding: "6px 12px", borderRadius: 8,
                  border: `1px solid ${buttonsDisabled ? theme.border : theme.primary}`,
                  background: buttonsDisabled ? "var(--color-card, white)" : `rgba(124,58,237,0.06)`,
                  color: buttonsDisabled ? theme.muted : theme.primary,
                  fontSize: 12, fontWeight: 500,
                  cursor: buttonsDisabled ? "not-allowed" : "pointer",
                  opacity: buttonsDisabled ? 0.5 : 1,
                  transition: "all 0.2s",
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  if (!buttonsDisabled) {
                    e.currentTarget.style.borderColor = theme.primary
                    e.currentTarget.style.background = `rgba(128,90,213,0.06)`
                    e.currentTarget.style.color = theme.primary
                  }
                }}
                onMouseLeave={(e) => {
