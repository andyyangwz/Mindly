...
export default function FocusTimer() {
  ...
  const { timeLeft, setTimeLeft, running, setRunning, hours, minutes, seconds, display } = useTimer(durations[session]);
  const [totalTime, setTotalTime] = useState(durations[session]);
  const percent = timeLeft / totalTime * 100;
  const circlePercent = running ? percent : 100;
          ...
          <div style={{ position: "relative", width: s, height: s, marginBottom: large ? 28 : 16 }}>
            <svg width={s} height={s} style={{ transform: "rotate(-90deg)" }}>
              ...
              <circle cx={cx} cy={cx} r={r} stroke="white" strokeWidth="6" fill="none" strokeLinecap="round"
                strokeDasharray={`${circ}`} strokeDashoffset={`${circ * (1 - circlePercent / 100)}`} />
            </svg>
