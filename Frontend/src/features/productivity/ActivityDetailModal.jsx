...
import { Edit3, Trash2, Clock, Flag, Play, X, CheckCircle2, Circle, RotateCcw } from "lucide-react"
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 28 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                ...
                <span style={{ fontSize: 13, color: viewingActivity.color, fontWeight: 500 }}>
                  {viewingActivity.isDeadlineMarker ? (
                    <>{t("productivity.event.due")} {formatDateTime(viewingActivity.eventDate, viewingActivity.startTime)}</>
                  ) : viewingActivity.hasDeadline ? (
                    <>{t("productivity.event.task")}</>
                  ) : viewingActivity.startTime ? (
                    <>{formatDateTime(viewingActivity.eventDate, viewingActivity.startTime)} &ndash; {formatTime(viewingActivity.endTime)}</>
                  ) : viewingActivity.eventDate ? (
                    <>{new Date(viewingActivity.eventDate + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}</>
                  ) : null}
                </span>
              </div>
            </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 24 }}>
            ...
            {viewingActivity.productivityLevel && (
              <span style={{ fontSize: 11, background: `${PRODUCTIVITY_LEVEL_COLORS[viewingActivity.productivityLevel]}14`,
                color: PRODUCTIVITY_LEVEL_COLORS[viewingActivity.productivityLevel], borderRadius: 20, padding: "5px 14px", fontWeight: 500,
                display: "flex", alignItems: "center", gap: 5,
              }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: PRODUCTIVITY_LEVEL_COLORS[viewingActivity.productivityLevel], flexShrink: 0 }} />
                {t(`productivity.eventForm.level_${viewingActivity.productivityLevel}`)}
              </span>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "#6366F108", borderRadius: 10, border: "1px solid #6366F120" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: theme.dark, fontWeight: 500, margin: 0 }}>{formatDateTime(viewingActivity.eventDate, viewingActivity.startTime)}</p>
                </div>
              </div>
